...
def expand_node(node):
    ...
    expanded_nodes = [node for node in expanded_nodes if node.state is not None]  # list comprehension!
    return expanded_nodes


def dfs(start, goal, depth=10):
    start_node = create_node(start, None, None, 0, 0)
    fringe_stack = [start_node]
    current = fringe_stack.pop()
    path = []
    while current.state != goal:
        temp = expand_node(current)
        for item in temp:
            fringe_stack.extend(expand_node(item))
        current = fringe_stack.pop()
        if current.depth > 10:
            return None
